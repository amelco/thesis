...
The flow equations that deal with the movement of water in the soil can be applied for saturated and non-saturated conditions. The structure of the soil is very complex, its pores forms tortuous paths and dead-ends for the water flow, turning impractical a microscopic study of its flow (A SHORT INTRODUCTION HERE)
...
$$
%
known as Darcy's law. $Q$ [L$^3$T$^{-1}$] is the water flow, $A$ [L$^2$] is the cross-section area of the column, $\Delta H$ [L] is the difference in hydraulic potentials of the beginning and the end of the column with length $L$ [L] and $K_s$ [LT$^{-1}$] is a proportionality factor called saturated hydraulic conductivity \cite[hillel].
...
Although it is an empirical equation, it can be derived through the relationship between Bernoulli and Poiseuille's laws. For this derivation, see \citeonline[warrick]. 

The water flux density $q$ [LT$^{-1}$] (also called Darcian velocity) represents the volumetric water flow per unit area and can be generalized to represent the flow in the three spacial dimensions by its differential form \cite[hillel,warrick,libardi]:
\label[water_flux]
$$
...
The Darcy's law is valid only for laminar flow. At high fluxes (or velocities) $q$ the hydraulic gradient eventually becomes nonlinear (). 

\medskip
\label[fig_Re]
\picw=10cm \cinspic Re1.pdf
\caption/f {Darcy's law deviation at high flux. The continuous line is the actual flux and the dashed lines the Darcy's flux}
\medskip
{\tenpoint Source: \citeonline[hillel]}
