...
Solute mobility in soil is accounted by two main processes: 1) convective transport by water mass flow through the transpiration stream and 2) movement driven by diffusion that depends on the solute concentration gradient in the soil caused by depletion of solutes due to root uptake \cite[barber74]. 
The first to recognize that solutes move towards the root due to the uptake process was \citeonline[bray].
Since then, several models were developed solving the CDE in an analytical way and, later, with the improvement of computational power, numerical models could be developed.
Analytical models ADVANTAGES/DISADVANTAGES whilst numerical models ADVANTAGES/DISADVANTAGES.
