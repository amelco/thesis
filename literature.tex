...
The MM equation is supposed to describe well the solute uptake for both anions \cite[epstein72,siddiqi,wang] and cations \cite[kochian,kelly,sadana,broadley,lux] in the low concentration range and, adding a linear component ($k$) to the equation, it can properly estimates the uptake rate also for higher concentrations \cite[epstein72,kochian,borstlap,wang,vallejo,broadley].
...
Moreover, experiments have shown that a minimum concentration ($C_{min}$) bellow which the uptake ceases is commonly found \cite[mouat,dudal,machado].
...
Measuring $C_{min}$ is particularly difficult \cite[seeling,lambers] because it occurs at very low concentration levels that may be hard to be accurately measured. \citeonline[seeling] shows that $C_{min}$ can be neglected for the cases of high $K_m$ values.
