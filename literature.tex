...
The values of MM parameters are strongly dependent on the experimental methods used and vary with plant species, plant age, plant nutritional status, soil temperature and pH (). Therefore, they have to be determined for each particular experimental situation.
Some types of experiments to determine the kinetic parameters $I_m$, $K_m$ and $C_{min}$ include hydroponically-grow plants () and the use of radioisotopes to estimate them directly from soil (). The later is more realistic since there is a large difference between a stirred nutrient solution and the complex and dynamic soil medium.
...
Therefore, the original MM equation is often modified to consider either the linear component due to high concentrations and the concentration where uptake stops:
...
Figure \ref[MM_plus] shows the effects of both new parameters on the original MM equation. The Equation \ref[eq_MM_plus] is used in many solute uptake models () instead of the original equation.
