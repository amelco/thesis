...
%Plants need to uptake water and nutrients from soil to
 ...
 \citeonline[epstein52] suggested that the rate of solute uptake by plant roots can be referred to be similar to that between enzyme and its substrate. They used the  Michaelis-Menten equation \cite[michaelis] to define the net influx of a solute and its concentration in external medium, for potassium and sodium solutions in barley roots. The most well-known form of their equation is the following:
$$
I = {I_m C \over K_m+C}
$$
%
where $I$ is net influx (rate of solute uptake), $I_m$ is the maximum uptake rate, $C$ is the solute concentration in external medium and $K_m$ the Michelis-Menten constant. $I_m$ and $K_m$ are found experimentally, being $K_m$ the concentration value at which $I_m$ assumes half of its value and represents the affinity of the plant for the solute. The uptake rate following the MM kinetics is an asymptote which saturates with increasing external concentration (Figure \ref[orig_MM]).
...
The original MM equation also describes the solute uptake for other solutes including anions \cite[epstein72,wang] and cations \cite[kelly,sadana,broadley,lux]. 
Experiments has shown that, for low concentration in the external medium, a minimum concentration value in which the uptake ceases is likely to occur \cite[mouat,dudal,machado] and for the high concentration range, a linear relationship between uptake rate and external concentration is often found \cite[epstein72,borstlap,wang,vallejo,broadley]. 
The MM parameters depends on plant and solute species, plant age, pH and temperature, turnig them difficult to be measured... IMPROVE
Finding the MM parameters are hard and has to be made with caution. Adding another parameter is a great limitation, even more because $C_lim$ value is hard to be measured \cite[seeling,lambers] and can be negleted for cases of high $K_m$ values \cite[seeling].
Thus, a linear component and/or the minimum concentration value are commonly added to the MM equation in more recent solute uptake models \cite[barber,and,all,others].  

\medskip
\label[orig_MM]
\picw=13cm \cinspic orig_MM.pdf
\caption/f {Solute uptake rate as a function of external concentration by Michaelis-Menten kinetics}
\medskip
