...
They are usually classified as being microscopic or macroscopic models, according to the considered scale \cite[feddes]. 
...
One of the advantages of the microscopic approach is that it automatically allows water uptake compensation, as the computed local water potential gradients control the uptake for the whole root system \cite[simunek]. 
...
Another classification of models, relates to the experimental approach of the phenomena, distinguishing between empirical and mechanistic models. 
Empirical models \cite[chanter,ross,yerokun] are based on the response of a set of experiments and attempt to describe the observed phenomena without hypothesizing on underlying mechanisms. 
...
The main disadvantage of this kind of model is that the model parameters do note have a clear physical, physiological or biological meaning, limiting interpretation and substantial model improvement. 
...
Mechanistic models \cite[barber74,cushman,willigen1,roose,simunekHYDRUS,somma,vandam] seek to explain the physical mechanisms that drive the phenomena, requiring a better understanding and mathematical description of the underlying processes. 
...
Measuring $C_{min}$ is particularly difficult \cite[lambers,seeling] because it occurs at very low concentration levels that may be hard to be accurately measured. \citeonline[seeling] shows that $C_{min}$ can be neglected for the cases of high $K_m$ values.
...
Authors do not seem to agree on the use case of MM equation.  
Depending on the assumptions made in the model, some authors use it to deal with both active and passive uptake while others use it to deal with active uptake only \cite[shi].
...
\citeonline[silberbush],
%102.pdf on calcium uptake in a soilless culture, Schröder et al. (2012) 
\citeonline[schroder], 
%48.pdf, Ungs, Boersma and Akratanakul (1982) 
\citeonline[ungs], 
%11.pdf, Raij et al. (2013) 43.pdf 
\citeonline[raij] and 
%Simunek and Hopmans (2009) 
\citeonline[simunek]. 
%68.pdf just to cite some.
}
...
\citeonline[dalton] proposed a physical-mathematical model that includes active uptake, mass flow and diffusion, and expresses solute uptake as being proportional to root water uptake. 
...
Nevertheless, MM equation is the most frequently used equation in root solute uptake models for being physically grounded and for its good agreement with experimental data.
...
Combined water and solute uptake models compute averages or local pressure and osmotic heads, allowing to estimate plant water and osmotic stress. 
