...
Knowing that $A_s=\pi {r_m}^2$, Equation \ref[eq_devR] can be written as
\label[eq_devR2]
$$
...
With Equations \ref[eq_devrm] and \ref[eq_devrm2] we have basic relationships that can be used to find the expression for any parameter that are present at those equations, since the values of the other parameters are not missing.
Thus, for a known value of $R$, we can calculate $r_m$ and $L$ (Equations \ref[eq_rm] and \ref[eq_L]) using Equation \ref[eq_devrm] and by solving Equation \ref[eq_devrm2] for $L$, respectively. The later leads to:
$$
L = {A_p z \over \pi {r_m}^2} 
$$

As mentioned in Section \ref[sec_micro], when there is no root length density data available (or other parameter that can lead to it), $L$ can be estimated by measuring some root and soil characteristics.
Equation \ref[eq_L2] has all needed parameters to calculate $L$:
$$
...
The total volume of a parcel is the sum of soil and root volumes. 
It is easier to find the root volume by difference between total and soil volumes. 
The first can be found by multiplying plant area by root depth and the later by the relation between volume, mass and density. 
Hence, volume of roots can be calculated as follows:
$$
V_t = V_r+V_s \Rightarrow V_r = V_t-V_s = A_p z - {m_s \over d_s} \eqmark
$$

Also, root volume can be calculated as the cylindrical root volume:

$$
V_r = \pi \overline{r_0}^2 L \eqmark
$$

The unknow is $L$, all other parameters are measured. Relating the two $V_r$ equations we can solve for $L$:
$$
