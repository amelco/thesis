...
This particular analytical model was chosen due to the reason that it also uses the MM equation as the boundary condition at root surface to deal with solute uptake.
...
The simplification was based on the assumption that the concentration in solution is initially low, then the nonlinear MM equation becomes linear:
...
The nondimensional variables $\eta$, $\mu$, $\tau$, $\Theta$, $v$ and $a$ account for time, distance, root ability to absorb solute by convective flux, solute concentration in soil solution, ratio of convective to diffusive flux and outer influence of the root, respectively.
It is important to know that $\tau=k/q_0$, where $k$ is the root absorbing power, or $K_m$ in the nomenclature of this thesis.
...
In this thesis, it was implemented an increment of 1\% of the following selected parameters ($P$): a) MM parameters $I_m$, $K_m$; and  b) soil hydraulic parameters $\alpha$, $n$, $\lambda$, $K_s$, $\theta_r$ and $\theta_s$.
