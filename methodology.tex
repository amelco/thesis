...
Computational implementations of the analytical models of \citeonline[cushman] and \citeonline[willigen1] were made to confront them with the numerical solutions. The former uses the MM equation as the boundary condition at root surface to deal with solute uptake.
...
The default values for $\Delta r_{min}$, $\Delta r_{max}$ and $S$, on Equation \ref[eq_disc], are of 10$^{-5}$ m, 5~10$^{-3}$ m and 0.5, resulting in XXX, 68 and XXX segments for the high, medium and low root density simulations, respectively.
To guarantee complete convergence for the non-linear model, the time step for $C_0$ values less than \clim{} was set to 0.01 s.
{\localcolor\Red
