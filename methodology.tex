...
* Solute uptake by mass flow of water is not controlled by the plant by other means but transpiration;
* Plant demand is equal to the $I_m$ parameter from MM equation.
\enditems

We assume that the plant demand for solute is constant, but not the uptake, in a way that solute uptake can be either greater or lesser than the demand, depending on the concentration in the soil solution at root surface. 
...
Hence, a situation where the plant demand is diminished due to an excess of solute is not considered, which can lead to overestimation of the uptake.

A piecewise uptake function that considers the explicit assumptions was, then, formulated:
\label[eq_MM_mod]
\numcases{q_{s0} =}
{I_m C_0 \over K_m+C_0}+q_0 C 	&if $C_0 < C_{lim}$, \\
\label[eq_case2]
...
A linearization of the MM equation was formulated... \ref[eq_case2], \ref[eq_case3]
...
\caption/f {Solute uptake piecewise equation \ref[eq_MM_mod] after assumptions made over original MM equation \ref[eq_MM1].}
\medskip
