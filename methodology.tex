...
The extraction area was then subdivided into $n$ circular segments of size $\Delta_r$, small near the root and increasing with distance, according to the equation:
\label[eq_disc]
$$
\Delta_r = \Delta r_{min}+(\Delta r_{max}-\Delta r_{min}) \left( {r-r_0 \over r_m-r_0} \right)^S \eqmark
$$
%
where the subscripts in $\Delta_r$ indicate minimum and maximum sizes defined by the user and $S$ is the rate at which the segment size increase. 
...
$$
%
and it is solved by an implicit numerical scheme as follows:
$$
\eqalignno{
\label[eq_disc_sol]
...
$$
q_s = -D(\theta) {\partial C \over \partial r} + q C = 0\;{\rm ,} \quad t>0,\; r=r_m \eqmark
$$
...
* Plant demand is equal to the $I_m$ parameter from MM equation.
* There is a soil solution concentration value (\clim) that the solute flux limits the uptake;
\enditems
...
%IMPROVE
In that case, we assume that active uptake is zero and all uptake is done by passive process. A concentration \c2 for this situation is calculated.
%--
...
Hence, a situation where the demand is diminished due to an excess of solute inside the plant is not considered, which can lead to overestimation of the uptake.
...
Finally, the boundary condition at root surface can be writen in a implicit numerical scheme as:
...
%IMPROVE
It is important to notice that, for concentrations values between \clim{} and \c2, active uptake it is not at its potential value because the passive contribution to the uptake already fills a portion of the plant demand. The plant will conserve this additional energy.
...
It was chosen to use the modified MM equation because it the uptake rate is governed by plant demand until the solute flux to the root is limiting the uptake. In that case, the solute uptake is governed by the solute flow towards the root.
...
The linearization proposed in the model is very si,ilar to that of Tinker and Nye (pg.116)
...
USE IT: In this thesis, we will consider $H$ as being the sum of pressure and osmotic heads ($h$ and $h_\pi$, respectively) since the gravitational component can be neglected due to the scale of the chosen domain (region around a single root with XXX m$^2$).
