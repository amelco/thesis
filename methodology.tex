...
Analogous to \citeonline[vandam_feddes], Equation \ref[eq_complete_richards] can be solved using an implicit scheme of finite differences with the Picard interactive process:
$$
\eqalignno{
\label[eq_disc_water]
C_i^{j+1,p-1} (H_i^{j+1,p}&-H_i^{j+1,p-1}) + \theta_i^{j+1,p-1}-\theta_i^j = {t^{j+1}-t^j \over r_i \Delta r_i} \cr
&\left[ 
