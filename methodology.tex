...
$$
%
where $C\!L_t$, $C\!N\!L_t$, $AcL_t$ and $AcN\!L_t$ are the solute concentration in soil water and the accumulated uptake for LU and NLU, respectively,  
and $x$ can be the time ($t$) or the distance from the axial center ($r$).
%and $t_{end}$ is the end time of the simulation.
...
Also, the Mann--Whitney U test was made with two datasets (concentration and cumulative uptake over time) for the two models. 
