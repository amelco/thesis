...
Cumulative relative difference

$$
\label[error_rel]
diff_{rel} = {\sum_{t=1}^{t_{end}} |C\!L_t - C\!L\!U_t| \over \sum_{t=1}^{t_{end}} C\!L\!U_t} \eqmark
$$
%
where $C\!L_t$ and $C\!L\!U_t$ are the solute concentration in soil water for LU and NLU, respectvely, at a given time $t$, and $t_{end}$ is the end time of the simulation.
