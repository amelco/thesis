...
% At the end of use, user must set \dsubnum=0
\newcount\dsubnum
\dsubnum=0
...
* There is a soil solution concentration value that the solute flux limits the uptake;
* Solute uptake by mass flow of water is not controlled by the plant by other means but transpiration.
\enditems

We assume that the plant demand for solute is constant, but not the uptake, in a way that solute uptake can be either greater or lesser than the demand, depending on the concentration in the soil solution. 
If the concentration is bellow a certain limiting value ($C_{lim}$), the uptake is limited by the solute flux, {\it i.e.} solute flux can not attend plant demand.
...
Therefore, a piecewise uptake function that considers the explicit assumptions was formulated:
% Solve problem with alignment of equation numbers
$$ 
q_{s0} = 
\cases{
  \label[eq_MM_case]
  {I_m C_0 \over K_m+C_0}+q_0 C 	&if $C_0 < C_{lim}$ \eqsubmark\cr 
  I_m 					&if $C_{lim} \leq C \leq C_2$ \hfill\eqsubmark\cr
  q_0 C 				&if $C > C_2$  \hfill\rlap\eqsubmark\cr
}
$$
\dsubnum=0


A linearization of the MM equation was formulated... 
