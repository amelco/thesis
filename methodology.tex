...
A computational implementation of the analytical model of \citeonline[cushman] was made to confront it with the nonlinear numerical solution (NLU). 
This particular analytical model was chosen due to the reason that both models use the MM equation as the boundary condition at root surface to deal with solute uptake.
A transcription of the Cushman's equation can be found here:
%but the reader can find all the details in his original work.

$$
\Theta(\mu,\eta) = \sum_{n=0}^\infty A_n \mu^v \beta(\mu,\tau,\alpha_n) \exp(-\alpha^2\eta)+1 \eqmark
$$
where
$$
\eqalignno{
\beta_v(\mu,\tau,\alpha_n) &= \left[\alpha aY_{v-1}(a\alpha)-2v(1-\tau)Y_v(\alpha_n)\right] J_v(\alpha_n\mu) & \cr
                         &- \left[\alpha aJ_{v-1}(a\alpha)-2v(1-\tau)J_v(\alpha_n)\right] Y_v(\alpha_n\mu),
}
$$
the $\alpha_n$ are roots of
$$
\eqalignno{
&[\alpha aJ_{v-1}(\alpha a)-2vJ_v(\alpha a)] [\alpha Y_{v-1}(\alpha)-2v(1-\tau)Y_v(\alpha)] + & \cr
&[2vY_{v}(\alpha a)-\alpha aY_{v-1}(\alpha a)] [\alpha J_{v-1}(\alpha)-2v(1-\tau)J_v(\alpha)] = 0,
}
$$
and the $A_n$ are given by
$$
A_n=-{\displaystyle\int_1^a \mu^{1-v}\beta_v(\mu,\tau,\alpha_n)d\mu \over \displaystyle\int_1^a \mu{\beta_v}^2(\mu,\tau,\alpha_n)d\mu}.
$$

This analytical solution is for a linear boundary condition. 
The simplification was based on the assumption that the concentration in solution is initially low, then the nonlinear boundary
$$
D{\partial C \over \partial r}+q_0 C = {kC \over 1+kC J_{max}}
$$
becomes linear
$$
D{\partial C \over \partial r}+q_0 C = kC.
$$
...
The simulation scenarios were chosen in a way that only one input parameter is changed, taking scenario 1 as default. 
Thus, the variation in soil hydraulic properties is accounted in scenarios 1, 6 and 7; root density in scenarios 1, 4 and 5; initial concentration in scenarios 1 and 3; and in potential transpiration in scenarios 1 and 2.
...
To analyze the differences between the two proposed models (linear and nonlinear solutions), the relative differences in the predicted concentrations ($di\!f\!\!f_{C}$) and accumulated uptake ($di\!f\!\!f_{Ac}$), for both models, were calculated as follows:
$$
\eqalignno{
\label[error_C]
...
It was computed the relative difference between three outputs: two in relation of time -- concentration at root surface $C_0(t)$ and accumulated solute uptake $Ac(t)$ -- and one in relation of radial distance -- concentration $C(r)$.

%The analysis of the results was made in order to choose one out of the two models in further simulations. 
NLU solution uses the non-linear MM equation \ref[eq_MM_mod] and, due to an additional iterative process in the numerical implementation, it takes longer to compute the results when compared with the linear solution LU. 
It is also susceptible to stabilization issues in the results, depending on the selected time and space steps. 
LU is a simplified version of the MM equation in a way that the solute uptake rate for the situation $C_0 < C_{lim}$ is always smaller then that of the original nonlinear equation (details in Appendix \ref[ap_l_nl_fluxes]). 
It has no problems regarding to stabilization and it is faster because it is less sensitive to space and time steps. 
...
The objective of this first analysis is to check if the difference in the results generated by the linearization of MM equation is sufficiently large to be properly analysed.  
For that, four different general scenarios were chosen (scenarios 1 to 4 as listed in Table \ref[tab_scenarios]).
...
The Mann--Whitney U test \cite[mcknight] was made with three datasets: concentration $C_0(t)$ and accumulated uptake $Ac(t)$ over time, and concentration over radial distance $C(r)$) for the two models and for each scenario. 
The choice of this test is due to the fact that the distributions for both datasets are not normal, the pairs (datasets for LU and NLU) are distinct and do not affect each other. 
This test can decide whether each pair is identical or not without assuming them to follow the normal distribution (nonparametric test). 
The null hypothesis ($H_0$) is that both groups 
%(model output data) 
does not differ from each other and the alternate hypothesis ($H_1$) is that one group is different.

With this test, it is possible to decide in which cases one has to use a particular model (in case they are different) or if both models are equally useful (in case they present no statistical difference).
...
To find the sensitivity of the model to a input parameter is to find the magnitude of its derivative in respect of the model result. If the derivative, for instance, is zero, the model has no sensitivity to this parameter. The higher the derivative, the higher is the sensitivity and, therefore, the higher is the precision needed to measure that parameter. By making a relative analysis, one can observe the sensitivity from distinct parameters through the same scale, and compare them. 

Here, we did an increment of 1\% of the following selected parameters ($P$): a) MM parameters $I_m$, $K_m$; and  b) soil hydraulic parameters $\alpha$, $n$, $\lambda$, $K_s$, $\theta_r$ and $\theta_s$.
The analysed predicted variables ($Y$) were: end time of simulation $t_{end}$, osmotic head in the end of simulation $h_{\pi}$, pressure head in the end of simulation $h$, average osmotic head in the soil profile in the end of simulation $\overline{h_{\pi}}$, average pressure head in soil profile in the end of simulation $\overline{h}$ and accumulated uptake in the end of simulation $Ac$.
%
