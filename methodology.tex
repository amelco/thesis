...
A fully implicit numerical treatment was given to the Equations \ref[eq_Richards] and \ref[eq_sol].
...
The discretization needed for the numerical solution was made in the single root level. As the extraction properties of the root are considered uniform along its length, and assuming no vertical differences in root density and fluxes, the cylinder can be represented by a circle. 
The extraction area was then subdivided into $n$ circular segments of size $\Delta_r$~(m), small near the root and increasing with distance, according to the equation \cite[liersolute]:
\label[eq_disc]
$$
\Delta_r = \Delta r_{min}+(\Delta r_{max}-\Delta r_{min}) \left( {r-r_0 \over r_m-r_0} \right)^S \eqmark
$$
%
where the subscripts in $\Delta_r$ indicate minimum and maximum sizes defined by the user and $S$ gives the shape of the rate at which the segment size increase. 
This approach for the discretization has the advantage of determine smaller segments in regions that need more precision in the calculations (in this case, closer to the root surface) due to the greater variation of the flux properties. 
...
\caption/f {Schematic representation of the discretized domain considered in the model}
\medskip
...
%IMPROVE
In that case, we assume that active uptake is zero and all uptake is done by passive process. A concentration \c2 (mol) for this situation is calculated.
%--
...
%IMPROVE
Assumption 1 states that passive uptake is not regulated by physiological plant mechanisms and, in order to save metabolic energy, the plant regulates active uptake as a complementary mechanism of extraction to achieve plant demand, making the active contribution to uptake be less than its potential value. 
...
The effects of solute concentration inside the plant in solute uptake and plant demand are not considered in the model.
Hence, a situation where the demand is diminished due to an excess of solute inside the plant is not considered, which can lead to overestimated predictions of the uptake.
...
$$
%
where $\alpha$ is the active contribution for the solute uptake slope $(\alpha+q_0)$.
%IMPROVE
The linearization proposed in the model is very similar to that of Tinker and Nye (pg.116), but does not considers concentration inside the plant.
...
Here, we present the numerical discretization and the applied boundary conditions, together with the algorithm to find the solution for Equation \ref[eq_complete_solute].
%find a better place to put it
Both water and solute flow are considered to have transient condition. 
%--
In this thesis, we will compare the proposed model with the original one \cite[liersolute], and  with the analytical models of constant uptake \cite[willigen1] and concentration dependent (MM) uptake \cite[cushman], both considering steady-state condition with respect to water flow. 
A numerical implementation of the constant uptake was made, therefore, the numerical solutions for the concentration dependent, zero uptake and constant uptake models are shown.

%MODIFY IT. Copyied from quirijn
The combined water and salt movement is simulated iteratively. In a first step, the water movement towards the root is simulated, assuming salt concentrations from the previous time step. In a second step, the salt contents per segment are updated and new values for the osmotic head in all segments are calculated. The first step is then repeated with updated values for the osmotic heads. This process is repeated until the pressure head values and osmotic head values between iterations converge. Flowcharts containing the algorithm logic is shown in the Appendix \ref[ap_flowchart].
...
Applying boundary conditions of nonlinear concentration dependent solute flux, the first and second term of the right-hand side of Eq. \ref[eq_disc_sol] are equal to $-({I_m \over 2 \pi r_0 L(K_m +C^{j+1}_1)} + q_0)C^{j+1}_1 \Delta r_1$ while $C < C_{lim}$ and $C > C_2$:
$$
\eqalignno{
\label[NLU]
\theta^{j+1}_1 &C^{j+1}_1 - \theta^j_1 C^j_1 = {\Delta t \over 2 r_1 \Delta r_1} \times \cr
& \left\{  
{r_{1-1/2} \over r_{1}-r_{0}} \left[-({I_m \over 2 \pi r_0 L(K_m +C^{j+1}_1)} + q_0)\right] C^{j+1}_1 \Delta r_1 \right. - & \eqmark \cr
& \left. { r_{1+1/2} \over r_{2}-r_{1}} \left[ q_{1+1/2}(C^{j+1}_{1}\Delta r_{2} + C^{j+1}_{2}\Delta r_{1}) - 2D^{j+1}_{1+1/2}(C^{j+1}_{2}-C^{j+1}_{1}) \right] 
\right\} & 
}
$$
...
The zero uptake solution is the original model presented by \citeonline[liersolute]. The numerical discretization is according to Equation \ref[eq_disc_sol] and the intermediate nodes are analogous to Equations \ref[eq_int_nodes1] to \ref[eq_int_nodes3]. The only difference is the boundary at root surface.
...
The constant uptake solution is was proposed by \citeonline[willigen1]. The numerical discretization is according to Equation \ref[eq_disc_sol] and the intermediate nodes are analogous to Equations \ref[eq_int_nodes1] to \ref[eq_int_nodes3]. The only difference is the boundary at root surface.

\seccc The inner boundary ($i=1$)

Applying boundary conditions of constant solute flux, the first and second term of the right-hand side of Eq. \ref[eq_disc_sol] are equal to $-{I_m \over 2 \pi r_0 L}\Delta r_1$ while $C > 0$:
$$
\eqalignno{
\label[will]
\theta^{j+1}_1 &C^{j+1}_1 - \theta^j_1 C^j_1 = {\Delta t \over 2 r_1 \Delta r_1} \times \cr
...
\sec Analytical implementations of convection-dispersion equation
