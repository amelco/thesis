...
As mentioned in the previous chapter, the models for water and solute uptake can be divided in macroscopic and microscopic. 
...
Microscopic models consider a single cylindrical root of radius $r_0$ \uhead with an extraction zone being represented by a concentric cylinder of radius $r_m$ (m) that bounds the half-distance between roots. The height of both cylinders is $z$ \uhead and represents the rooted soil depth. 
...
The substitution of Equation \ref[eq_watflux] into \ref[eq_Richards] results in a second-order partial differential equation and, therefore, it needs initial and boundary conditions to yield a particular solution. 
...
In analytical solutions, steady-state conditions with respect to water flow is often used to solve the equation. The high non-linearity of the hydraulic functions (see Section \ref[soil_prop]) makes a transient solution rather complex and, sometimes, impractical.
In numerical solutions, on the other hand, it is easier to define a transient water flow condition, which allows a more realistic treatment to the equation. 
...
In order to deal with solutes, the osmotic head ($h_\pi$) must be added to $H$ and it will serve as a {\localcolor \Red `link'} to the solute transport equation, detailed in Section \ref[sol_transp]. {\localcolor\Red\quic This is not clear -- say exactly what you mean.}
...
The soil hydraulic properties $K$, $\theta$ and $h$ are interdependent and, as mentioned in the previous section, their relation is highly nonlinear. 
%Equations \ref[eq_theta] and \ref[eq_K] show their interdependence and their noticeable nonlinearity. 
Among available models to describe soil hydraulic properties, the \citeonline[genuchten80] equation system is most frequently used:
...
Equations \ref [eq_Ds] to \ref[eq_Dm] are one of the several existing models to estimate the effective diffusion-dispersion coefficient. {\localcolor\Red\quic Better present only the first two terms; then say that the tortusosity can be estimated, for example, by the Millington-Quirk relation -- the one you use.} 
%Chapter \ref[literature] mentions other models that can be used for this purpose.


%boundary conditions
The boundary conditions types for solute transport are analogous to those presented for the case of water flow, expressed in terms of concentrations instead of pressure heads. 
...
The choice for one or another boundary condition will be made depending on how the solute uptake by the plant is considered in the model (further details in Section \ref[sol_uptake]).
% Improve that
%The outer boundary depends on the distance from root surface in which it is considered. 
For the outer boundary, a zero solute flux condition works well to simulate inter-root competition for solutes since the average distance between roots is 1 cm.
A constant concentration -- equal to the initial concentration -- can be used in scenarios where the outer boundary is considered at a sufficient distance from the root surface so that competition between neighbouring roots can be considered negligible.
...
The passage of water and solutes from the soil to the root, where plant physiological mechanisms may play a role, occurs at the root-soil interface.
...
For a zero solute flux condition at root surface, the solute will accumulate 
%there 
at the root surface
...
Differently, in the case of a constant or a variable solute uptake at root surface, an accumulation of solute will result only if the uptake is less than the convective flow towards the root surface. 
...
described by the MM equation, as seen in Section \ref[literature]. Thus the uptake shape function $\alpha(C)$ can be supposed to follow the concentration dependent MM kinetics, and considering $k$ equal to $I_m$ leads to:
\label[eq_MM1]
$$
...
\caption/f {Solute uptake rate as a function of external concentration according to the Michaelis-Menten kinetics using {\localcolor\Red common} parameters {\localcolor\Red\quic You might be more specific here.}}
\medskip

%An analytical solution to the Equation \ref[eq_solute_complete], using Equation \ref[eq_MM1] as a boundary at root surface and a zero flux at the outer boundary and considering a steady-state condition for water flow, is presented by \citeonline[cushman]. A solution for the same equation but considering a constant solute uptake as the boundary of root surface is presented by \citeonline[willigen1]. (THIS PART GOES TO LITERATURE REVIEW)
