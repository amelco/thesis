...
In order to deal with solutes, the osmotic head ($h_\pi$) must be added to $H$ and its value is calculated through
...
In microscopic models, $h_g$ is neglected due to its minor relevance.
...
Values of $D^0$ for ions or molecules can be easily measured and estimated by analytical equations for free water. Soil water content is often below its saturation point, thus, diffusion is often different from that of free water. 
The tortuosity factor of Equation \ref[eq_Dm] can be calculated, for instance, by the empiric Millington-Quirk relation as follows:
