...
In other words, the simulations showed that reducing $T_p$ might cause the same concentration profiles for every scenario, although at different times, if all other parameters remains the same.
...
\caption/f {Relative transpiration as a function of time, with different values of the selected input parameters (a) soil, (b) root density, (c) initial concentration, and (d) potencial transpiration. Lighter straight lines are the average concentration}
\medskip
...
{\tred LITTLE DISCUSSION

IMPROVE: It is interesting that, with the same values of $I_m$ and $K_m$, a whole different sets of active and passive contributions can be generated just because of changes in soil and atmospheric parameters. This gives light to the really complex system that is the soil-plant and its relations with water and solute uptake.}
...
The higher sensitivity to $\theta_s$ can be explained due to the fact that the model has a powerful (possibly overestimated) solute extraction at LUP, due to the lack of a plant regulated process for solute uptake at this phase.
