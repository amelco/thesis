...
\def\diff{{\it diff}}
According to Equations \ref[eq_MM_mod] and \ref[eq_MM_linear], the difference between LU and NLU occurs only at the condition of concentration values at root surface below $C_{lim}$, due to the different considered boundary condition of each model. For now on, the condition $C_0 < C_{lim}$ will be referred as limiting concentration condition, or LCC, for short.
...
As the limiting concentration condition occurred with very low solute uptake rates in all simulation scenarios, the amount of solute extracted by the roots at the end of simulation are nearly the same for both models (Figure \ref[accumxt]) in all scenarios. 
The relative transpiration is also little affected by the change of the boundary condition. The difference between the models starts to occur at values around 0.03 or less, close to the end of simulation that stops when $Tr \leq 0.0001$. At the LCC occurs a second reduction in the transpiration and can be observed by an abrupt decrease of the transpiration rate, being the NLU reduction smoother (Figure \ref[diff_tr]).  
...
These results means that although there is indeed a difference between the two models, they are important only if looked in a "time zoom". 
If the interest is the result of the whole time serie, or the concentration as a function of the radial distance, they are not important. 
Considering that the metabolic response of the plant for different levels of concentration at root surface is immediate, it is plausible to consider  NLU for the simulations since it is the more realistic model. On the other hand, if there is a delay of any kind in that response -- the plant is not immediately affected by a change in the rate of solute uptake -- or if the variable of interest is the concentration as a function of radial distance, it is reasonable to use LU. In this thesis we choose to use NLU since we are analysing the behaviour of the model and, for that, it is interesting to be as close as possible to the real phenomena.
