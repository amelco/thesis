...
{\tblue 
Parameter $\lambda$ affects the shape of the hydraulic conductivity function such as a higher $\lambda$ results in a more intense decrease of hydraulic conductivity with decreasing water content, resulting in a prolongated water uptake at $T_p$, increasing the time at the onset of $T_r$ falling rate phase.
On the other hand, the steeper decrease in hydraulic conductivity causes a more intense decrease in the water flux which, on your turn, decreases the end of the simulation time.
...
\def\descc{Gray line is CU with the constant of solute uptake rate $I_m$ {\tred 10 times higher {\it QM: Mas qual eh o valor original?}}}

\medskip
\label[fig_C0]
%\picw=11cm \cinspic cxt1.pdf
\picw=13cm \cinspic cxt_comp.pdf
\caption/f {Soil solution concentration at the root surface as a function of time for constant (CU), zero (ZU) and non-linear (NLU) uptake models. \descc}
\medskip
...
For the case of NLU (Figure \ref[fig_heads2]), an accumulation period does not develop due to the high solute uptake rate in LUP (as can be seen in Figure \ref[fig_qst_detail], all solute carried by mass flow of water is taken up).
...
The LU model may show a period of accumulation of solute near the root surface depending on the potential transpiration rate and solute uptake rate values.
...
{\tred The line in Figure \ref[fig_C] representing CU model for a tenfold higher uptake rate shows that the diffusion of solute away from the roots generates a higher concentration somewhere between $r_0$ and $r_m$ in the CU model {\it QM: Porque precisa aumentar em 10x? No NLU vc nao faz isso. Dah para comparar assim? Nao muito claro. Explicar melhor.}}, as opposed to the NLU model, which has no solute accumulation period and its concentration profile diminishes when approaching $r_0$.
...
\caption/f {Solute concentration profile of the analytical model by \citeonline[cushman] and the {\tred numerical models, with parameters of scenario 1, at time $t=30$ days. The grey line is the numerical result of the linear model {\it QM: Dizer claramente qual linha eh qual modelo numerico.}} }
\medskip
