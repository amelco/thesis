...
(1) the first is caused by the combined water and osmotic stress caused by the decreasing water flux towards the root when the total hydraulic head $H$ reaches the limiting value $H_{lim}$ of $-150$ m (as occurred in the \citeonline[liersolute] model);
and
...
As \clim{} is not an input parameter, the only way to increase it is decreasing water flux (Figure \ref[fig_climxq]) by changing the initial water content, for example.
...
%{\it QM: There must be literature on these mechanisms. Cite some.}
 ...
 used the model HYDRUS to simulate solute uptake considering active and passive uptake
