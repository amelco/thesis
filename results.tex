...
Although low or high values of potential transpiration cause changes in the length of the phases (LUP, CUP and NUP), no changes in the active and passive partitioning was detected with the variation of this parameter.
...
In a case that all the model parameters are the same (soil type, root density, ion type and solute uptake), it is expected the same results in different times.
...
When $T_r\approx0$ (end of simulation), $C_0$ (17.88~mol~m$^{-3}$) is higher than $C_{lim}$ (5.97~mol~m$^{-3}$) (Figure \ref[fig_C_r](a)), the active contribution ends in its maximum value (Figures \ref[fig_contrib_t] and \ref[fig_contrib_c]) and the NUP does not begin (Figure \ref[fig_contrib_c], scenario 7).
...
As high is the root density, higher is the steepness of the $T_r$ falling rating phase (Figure \ref[fig_Tr_t]). As a consequence, active uptake becomes more important with increasing $R$, as can be seen in Figure \ref[fig_contrib_c] (scenarios 1, 4 and 5).
...
{\tred The final concentration profile and relative transpiration are important model outputs to be analysed. 
The former is essential for the model upscaling since the average soil concentration needed in the calculations of the majority of the macroscopic models is used.
The later is related to biomass and yield predictions.}
The concentration profile can be checked at any time. 
Figure \ref[fig_C_r] shows it in the end of simulation. 
This result is important in the computation of the average concentration, essential for macroscopic models.
...
\caption/f {Concentration in soil water as a function of distance from axial center, at the end of simulation, with different values of the selected input parameters (a) soil, (b) root density, (c) initial concentration, and (d) potencial transpiration, simulated using the nonlinear uptake model (NLU). Lighter straight lines are the average concentration}
\medskip
