...
\def\desc{Scenarios according to Table \ref[tab_scenarios] with scenario 1 as default and the others with the variation parameter specified on their titles}

\medskip
\label[fig_contrib_t]
\picw=17cm \cinspic all_contributionsxt.pdf
\caption/f {Active and passive contributions to the solute uptake as a function of time. Thicker continuous line represents the total uptake (active+passive). \desc}
\medskip

\medskip
\label[fig_contrib_c]
\picw=17cm \cinspic all_contributionsxc.pdf
\caption/f {Active, passive and total uptake ($q_{s_0}$) as a function of concentration in soil water. \desc}
\medskip
...
A plant regulated process that limits the solute uptake (for example, a reflection coefficient) might be an alternative to regulate the uptake in this phase.
...
It is shown in Figure \ref[fig_contrib_c] different behaviours for the active and passive contributions for solute uptake, with respect to soil concentration on soil solution.
Although low or high values of potential transpiration cause changes in the length of the phases LUP, CUP and NUP, no changes in behaviour was detected with the variation of this parameter.
This result is straight forward to explain since the potential transpiration affects the rate of the potential water flux and uptake which, consequently, affects the rate of solute uptake.
...
{\tred EXPLAIN}
%{\tred The simulation scenario 7 presented different results when compared to the others due to clay soil. Figure \ref[fig_contrib_c] showns that the three phases are present in all scenarios, excepting scenario 7. 
In the clay soil, the water flux is small due to its hydraulic properties which causes also a small solute uptake. 
...
{\tred According to \citeonline[tinker], at low concentrations, the ion transport across membranes is limited by the diffusive flux of solute towards the membrane in case of high affinity uptake (low $K_m$ values) and

the contribution of the solute uptake by mass flow of water (through the transpiration stream) is estimated to be very low, often below 0.1\%.}
