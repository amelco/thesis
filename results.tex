...
{\tred 
The parameter $n$ showed the highest values of $\eta$ among the soil hydraulic properties, mainly for $h_{os}$ in the end of simulation reaching the value of $-$23 for scenario 2 (Figure~\ref[sensitivity_n]).
...
It shows that the same value of water content is reached with higher (more negative )values of pressure head.
This brings expectations that the limiting value $H_{lim}$ would be reached at lower values of $\theta$ and with higher (more negative) values of solute concentration in soil water ($h_\pi$), which
%That 
...
The cause for the lower value of $\eta$ in scenario 7 resides on the fact that the solute uptake rate in this scenario is very low and, even if the variation of 1\% in $n$ causes a bigger change on the retention curve, it does not strongly affect the solute uptake rate and, consequently, the change in $h_\pi$ (or $h_{os}$) at the end of simulation is small.
}
\medskip
\label[sensitivity_n]
...
(a) for the selected soils from Table \ref[soils] with dashed lines representing a variation of +1\% in parameter $n$;
(b) for the loam soil with dashed line representing a variation of +50\% in parameter $\alpha$; and
