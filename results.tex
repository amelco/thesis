...
The changing from equation 21 to equation 25 (change of boundary condition, from constant to nonlinear uptake rate) makes the numerical solution take some time to stabilize at the initial times if the time and space steps are not chosen properly. 
Many time and space steps combinations were used as an attempt to minimize the problem. 
Choosing a very small time step eliminates the stabilization problem but makes the simulation lasts longer. 
...
\caption/f {Changes in $C(r)$ according to different values of the selected parameters}
\medskip
