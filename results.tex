...
It is shown in the discussion of Section \ref[result_luxnlu] that there is no significant difference between LU and NLU models results of concentration profile.
Nevertheless, Figure \ref[fig_analytic] shows the concentration profile also for the LU model, considering that the analytical model has a linear solute uptake rate as the boundary condition at the root surface.
%As the analytical model is a solution to the Equation \ref[eq_cush] with a linear boundary condition of solute uptake at the root surface (Equation \ref[eq_bc_cush]), it is also shown in Figure \ref[fig_analytic] the result of the linear numerical model (grey line), which is similar to the nonlinear result (actually, according to the analysis made in section \ref[result_luxnlu], their difference is not significant).
The similar result indicates that the proposed numerical model is in agreement to 
{\tred
what is proposed in the literature, with the advantage of being more flexible because it has no limitations of steady-state condition, leading to a potential of simulate a wider range of scenarios.
}
