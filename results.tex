...
According to Equations \ref[eq_MM_mod] and \ref[eq_MM_linear], the difference between LU and NLU only occurs for concentration values at root surface below $C_{lim}$, due to the different boundary condition of each model. 
...
A possible explanation is that, at this point, the solute uptake rate decreases, building a more negative value for the osmotic head and, consequently, reducing also the water uptake rate.
Therefore, this reduction depends on the value of $q_0$, which directly affects the $C_{lim}$ value (according to Equation \ref[eq_clim]), and suddenly reduces the relative transpiration due to the coupled water and osmotic stress.
This limiting concentration, here only numerically computed, might have some physical or physiologic processes that controls it and, therefore, it needs to be investigated.
It can be seen in Figure \ref[diff_tr] that the nonlinear equation causes a smoother $T_r$ reduction, which seems to be more appropriate for a natural phenomenon.
...
The difference can be significant either by
a) increasing the value of $C_{lim}$ as such LCC period is achieved faster, or b) looking at the results by a different time scale. 
...
then, at the time of LCC, the water flux would be low and the LCC period 
%woulld be
still small. 
...
Table \ref[tab_diff] shows the relative difference between the two models for concentration at root surface as function  of time ($C_0(t)$), considering the shorter time scale only (beginning from the time where LCC starts), and for concentration as a function of distance from axial center ($C(r)$) at the end of simulation. 
...
In that case, it is preferred to use LU since a bigger time step can be used and the simulation runs faster.
On the other hand, by considering that the metabolic response of the plant for different levels of concentration at root surface is immediate -- only a slightly change in $C_0$ causes a instantaneous plant reaction -- or in situations where the value of $C_0$ at LCC is important, it is plausible to consider NLU for the simulations since it is a more realistic model.  
...
For different soil types, although the initial concentration in bulk soil is the same (10 mol m$^{-3}$), initially, the concentrations in soil water at the root surface are different, as shown in Figure \ref[fig_C_t](a). 
The explanation is that, as the input parameter for water content is {\it h}-based, its initial value is calculated according to Equation \ref[eq_theta], that leads to different $\theta$ values for each soil in accordance to their hydraulic parameters.
...
Active uptake reaches its maximum value when $C_0=C_{lim}$  and the uptake is then limited by the concentration in the soil in further time steps, starting the nonlinear uptake phase (NUP).
...
From the beginning of the simulations (wet soil, $T_r=1$), although it initiates in LUP, a constant (instead of a linear) uptake rate is developed. 
...
Therefore, according to the model assumptions, the rate of solute uptake at LUP is constant and its value depends on the potential transpiration, soil type, root length density, and initial concentration and it does not depend on ion type, since $q_{s_0}$ values are the same (Figure \ref[fig_contrib_t]).
...
%to explain 
since the potential transpiration affects the rate of the potential water flux and water uptake which, consequently, affects the rate of solute uptake.
In a case that all soil type, root density, ion type and solute uptake parameters are the same, the same results are expected but in different times.
The most significant difference in the partitioning between active and passive uptake can be noticed from the different soil types (specially on clay soil) and root density values (Figure \ref[fig_contrib_c]).
...
Consequently, $C_0$ is slowly depleted in CUP whereas $\theta$ rapidly reduces to $\theta_r$, causing a fast reduction of $q_0$ and $T_r$.
...
With high root densities, the same soil volume extracts more water in a given time period, causing a belated but fast reduction in $T_r$. 
...
By default, the concentration profile is set to be stored in a file at the end of each simulated day and in the end of simulation, but the user can select any time interval to it.
...
It is expected that decreasing $T_p$ values causes only changes in the times at which $C_0$ achieves limiting values ($C_2$ and \clim) due to the decrease in water and solute fluxes. 
...
Soil type and root density change the final concentration profile respectively due both to the different soil hydraulic properties that affects the water retention in soil and to the greater water and solute uptake per soil unit (as explained before in the active and passive uptake partitioning).
