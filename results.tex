...
The total head $H$ is not able to fall below $H_{lim}$ and, as the solute uptake rate ($q_{s0}$) decreases due to the limitation imposed by $C_{lim}$, a decrease in the water uptake rate ($q_0$) has to occur to maintain $H$ in its limiting value.
...
The $C_0$ value at the end of simulation is never absolutely zero due to the asymptotic behaviour of $q_{s0}$ function in response to decreasing $C_0$ values (Equation \ref[eq_case1]). 
...
The model is able to quantify the active and passive uptake (say its importance).
The time that the maximum active uptake value is reached depends on ... (say the importance of that)
The time that the concentration starts to decrease depends on ... and happens some time after the Tr reduction (this implies something interesting?)
...
\picw=17cm \cinspic final_every_bar.pdf
