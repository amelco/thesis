...
In addition, the LCC occurs at values of relative transpiration ($T_r$, the ratio between actual and potential transpiration) around 0.03 or less -- a condition in which the plant is at high (combined water and osmotic) stress levels -- and close to the end of simulation 
(that stops when $Tr \leq 0.001$).

{\tblue
Figure \ref[diff_tr] shows that transpiration reductions (abrupt decrease in the transpiration rate) may occur for the two models, resulting from two distinct processes:
(1) the first is caused by the combined water and osmotic stress resulted from the decreasing water flux towards the root when the total hydraulic head $H$ reaches the limiting value $H_{lim}$ of $-150$ m (as occurred in the \citeonline[liersolute] model);
and
(2) the second is due to the decrease of the solute uptake rate resulted from the change in the boundary condition when LCC starts, causing a decreasing (more negative) value for the osmotic head $h_\pi$.
...
As $H$ at the root surface is not allowed to reduce below $H_{lim}$, the model predicts lower (less negative) $h$ values resulting in smaller water flux rates at the root surface ($q_0$), reflected in $T_r$.
}
...
\caption/f {Relative transpiration as a function of time for scenarios 1 to 4, predicted by the linear (solid line) and the non-linear (dashed line) model. {\tblue Numbers inside circles represent the first and the second $T_r$ reduction}}
\medskip

{\tblue Differences between both model predictions can become more significant by increasing the value of $C_{lim}$ making LCC to be reached in a earlier stage and making its duration of occurrence be a bigger fraction of time.
%, or b) looking at the results on a different time scale. 
As \clim{} is not an input parameter, the only way to increase it, not changing the ion specie, is decreasing water flux (Figure \ref[fig_climxq]) by changing the initial water content, for example. Another alternative would be to consider another ion or plant specie, which would modify the $I_m$ and $K_m$ parameters.
Due to the nature of Equation \ref[eq_clim], the $C_{lim}$ value asymptotically grows, tending to infinity, with decreasing $q_0$.
Therefore, a value of $C_{lim}$ that would cause a more significant difference between LU and NLU is acquired with close to zero values of $q_0$, resulting in low (also close to zero) relative transpiration values. 
As a result of this dependence of $C_{lim}$ to the $q_0$ value, it was not possible to find any combination of initial parameters that would lead to a significant increase of the difference between LU and NLU. 
%We could not find a scenario that matched the first option {\tblue for the following reason: 
...
%However, even if the value of $q_0$ satisfies this condition, Equation \ref[eq_c2] shows that $C_2$ would be higher and, since the uptake function is a piecewise equation, $C_0$ would have to be decreased until it reaches $C_{lim}$ {\tblue for LCC to occur}.
...
%{\tblue {\it My answer: a linha de cima eh a diferenca absoluta, e o valor em porcentagem, a relativa}}
}
\medskip
...
{\tblue These results indicate that the difference in predictions of both models are relevant only during LCC,
...
In this thesis we chose to use NLU since the values of $C_0$ are important to determine, precisely, the active and passive contributions to the solute uptake, which will be analyzed in Section \ref[model_results].
...
The Von Neumann stability analysis \cite[neumann] shows that, for a hydrodynamical equation, a good criteria to chose the time step is $\displaystyle{\Delta t \leq {{\Delta r}^2 \over 2 D} }$ (using the symbology of this thesis).
...
The reason must reside on the fact that the criteria accounts for constant values of $D$ and $\Delta r$.
...
{\tblue In this section, the predictions of the ZU, the CU and the analytical model are shown in comparison to the non-linear model (NLU), for Scenario 1.}
...
The boundary conditions of the analytical model constrain it constant water uptake and dispersion coefficient (steady-state condition).
...
The differences between the two models is mostly due to the consideration of steady-state condition in respect to water flow in the analytical model 
{\tred\it QM: Para fazer uma comparacao justa, deveria rodar o modelo numerico para as condicoes de contorm do modelo analitoco. Assim, daria para, realmente comparar.}
{\tblue\it Andre: Nao foi possivel realizar simulacoes em steady-state para o fluxo de agua utilizando o modelo proposto. Uma nova equacao referente a nova condicao de contorno teria que ser resolvida e nao houve tempo habil para isso. Essa discussao, entao, nao tem validade?}
 as opposed to the proposed numerical model that deals with transient water and solute fluxes.
...
{\tblue CU results in a similar prediction with a slightly lower $C_0$ value because the convective solute flux towards the root is much higher than its constant uptake rate $I_m$, resulting in a solute accumulation at the root surface.
...
less solute will be predicted to accumulate at the root surface by the CU model and more similar CU and NLU predictions will be, as seen in the gray line of Figure \ref[fig_C0] (for demonstration purpose).
}

\def\descc{Gray line is CU with an arbitrary larger (10 times) value for the constant of solute uptake rate ($I_m = 2 \cdot 10^{-5}$ mol cm$^{-2}$ s$^{-1}$)}
...
For the case of NLU (Figure \ref[fig_heads2]), an accumulation period does not develop due to the high solute uptake rate in LUP (Figure \ref[fig_qst_detail]), resulting from the model assumption that all solute carried by mass flow of water (convective transport) that reaches the root surface is taken up.
...
For the case of CU and ZU, reduction of $T_r$ starts around the 21th day whilst for NLU it onsets on about the 24th day.
...
This result is compatible with the model assumption 1, stating that the plant does not control the solute taken up by mass flow of water.
Similar result was found by \citeonline[simunek] in simulations using a macroscopic model that considers active and passive uptake of solutes.
...
Additionally, \citeonline[robinson] concluded, in his review, that there is no limit for the uptake of some ions like NO$^-_3$ and K$^+$, making the predictions of the model in accordance to the phenomena.
...
A good amount of data can be found in \citeonline[robinson] and in \citeonline[willigen_thesis].
...
The CU model may show a period of accumulation of solute near the root surface depending on the potential transpiration rate and solute uptake rate values.
This period is then followed by a period of diffusion of solute away from the roots which, together with the solute uptake, are responsible for the depletion of solute at the root surface.
This behavior causes a different solute concentration profile at the end of simulation when compared with the proposed solute uptake model (NLU).
{\tblue However, CU and NLU models predict similar concentration profiles as the value of $I_m$ for CU model is increased.
The line in Figure \ref[fig_C] representing CU model for a arbitrary tenfold higher uptake rate shows similar predictions of concentration profiles, with differences due to the diffusion of solute away from the roots, resulting from the solute accumulation period that generates a higher concentration somewhere between $r_0$ and $r_m$ at the end of simulation.
...
\secc {\tblue Concentration at the root surface as a function of time \pmb{$C_0(t)$}}
...
\caption/f {{\tblue Relative transpiration and solute concentration at the root surface as a function of time predicted by the non-linear model for scenario 1. (1)~indicates the onset of the $T_r$ falling rate phase and (2)~indicates the onset of the $C_0$ falling rate phase}}
\medskip
...
A plant controlled process allowing the reduction of solute uptake
{\tblue 
(for example, a reflection coefficient that indicates how effectively the osmotic pressure of a solute can be exerted across a membrane \cite[nobel])
%{\it QM: Acho que voce ainda nao introduziu/mencionou esse termo, poderia explicar um pouco mais. Ou citar algum trabalho que faz isso}
}
might be a modeling alternative to describe controlled uptake in this phase.
In our model such a feature was not included and, according to model assumptions, LUP has a period of constant solute uptake rate and its value depends on the potential transpiration, soil hydraulic properties, root length density, and initial solute concentration.
...
During the CUP, the solute uptake by mass flow (passive uptake) does not attend completely the plant demand (Figure \ref[fig_phases]), thus, according to the model assumptions, an active uptake is developed and it becomes higher as passive uptake 
%(driven by mass flow of water) 
becomes lower in order to maintain the constant uptake rate.
It is a different approach than that of \citeonline[simunek] that use a assumption of independent responses of active and passive uptake.
In the proposed model, the constant demand at CUP, filled by active and passive uptake, is maintained until $C_0$ reaches $C_{lim}$ which, in turn, increases as $q_0$ decreases, according to Equation \ref[eq_clim].
...
that the time of the occurrence of the maximum value for active uptake depends on how fast passive uptake 
...
{\tblue In the clay soil, the solute uptake is smaller due to the smaller initial concentration in soil solution calculated according to its hydraulic parameters.
The low hydraulic conductivity of this soil makes the limiting potential $H_{lim}$ to be reached in a earlier stage, reducing the time at the onset of the $T_r$ falling rate phase and, consequently, the uptake caused by the convective transport (passive uptake).
As a result, the diffusive transport in the root surface (active uptake) quickly becomes dominant (Figure \ref[fig_contrib_c], scenario 7).
}
...
The higher root length density results in a steeper falling rate phase 
...
This modeling result conforms to the simulations performed by \citeonline[lierwater] that show the time of first occurrence of limiting water potential increasing with higher root densities.
...
The selected predictions $Y$ were: ending time of simulation ($t_{end}$), osmotic head ($h_{os}$), time at the onset of the $T_r$ falling rate phase ($t_{T_r}$), time at the onset of constant uptake phase ($t_{C_0}$) and accumulated solute uptake ($acc$).
Varying parameter $P$ were the ion/plant ($I_m$ and $K_m$) and the soil hydraulic ($\alpha$, $n$, $K_s$, $\lambda$, $\theta_r$ and $\theta_s$) parameters. 
%{\tred \it QM: Isso seria tipicamente M\&M.}
%{\tblue\it Andre: Est\'a em M\&M, repetido aqui.}
...
This is due to the fact that, with higher values of $I_m$, there are higher uptake rates of solute in both phases of constant and non-linear uptake, diminishing the value of concentration at the root surface in the end of simulation (less negative values of osmotic head $h_\pi$).
...
The reason is the small variation in $C_0$ during the simulation due to the low water and solute fluxes of the clay soil, explained in the previous section.

\medskip
\label[sensitivity_MM_rest]
\picw=15cm \cinspic final_everybar_MM_rest.pdf
\caption/f {Relative partial sensitivity of osmotic head at the root surface in the end of simulation ($h_{os}$) and accumulated solute uptake ($acc$) to MM equation parameters $I_m$ and $K_m$ for scenarios 1 to 7}
\medskip
...
Although the sensitivity results are presented separately for each parameter, output and scenario, the interactions between all parameters and simulated processes occur sinergistically and concomitantly.
Due to the relative nature of the sensitivity (there is no absolute value of $\eta$ for one prediction with a variation of different parameters), an individual analysis can give an insight in the importance of a parameter to the model, but it only represents the specific scenario.
Therefore, due to the high number of soil hydraulic parameters, the discussion will focus on those parameters with the greatest $\eta$ values.
...
Figure \ref[fig_wrcs] 
{\tblue shows how the water retention curve (a) and the hydraulic conductivity (b), following the \citeonline[genuchten80] model, change with the 
%{\tred variation of 1\% in $n$ parameter. {\it QM: N afeta ambas a retencaoo e a condutividade.Eh importante dizer isso tambem.}}
variation of the $n$ parameter.
}
...
However, the cause for the lower value of $\eta$ in scenario 7 might reside in the fact that the solute uptake rate in this scenario is very low.
Therefore, even with a variation of 1\% in $n$ causing a change in the retention curve (and consequently in the concentration values), it does not strongly affect the solute uptake rate and, consequently, the change in $h_\pi$ (or $h_{os}$) at the end of simulation is small.
...
\caption/f {(a) Water content from Equation \ref[eq_theta], for three combinations of values for $\alpha$ and $n$, and
(b) relative hydraulic conductivity from Equation \ref[eq_K] for three combinations of values for $\lambda$ and $n$, with $\alpha = 0.75$ m$^{-1}$
...
Increasing values of $\theta_s$ increases the available water content and increases at about the same proportion ($\approx$1\%) $t_{T_r}$, $t_{end}$ and $t_{C_0}$ due to a prolongated water uptake at $T_p$.
{\tblue 
...
On the other hand, the steeper decrease in hydraulic conductivity causes a more intense decrease in the water flux which, on your turn, decreases the end of the simulation time ($t_{end}$) value (Figure \ref[sensitivity_soil_time]).
...
As explained before, an increase in $\theta_r$ decreases the available water content, shortening the LUP phase and $t_{T_r}$.
As a result, the period of highest solute uptake rate is reduced causing, at the end of the simulation, a higher solute concentration at the root surface (more negative $h_{os}$).
...
An increase in its value makes that the same water content corresponds to a less negative pressure head (Figure \ref[fig_wrcs]a).
...
\caption/f {Relative partial sensitivity of osmotic head at the root surface in the end of simulation ($h_{os}$) and accumulated solute uptake ($acc$) to soil hydraulic parameters $\theta_r$, $\theta_s$, $\alpha$, $K_s$ and $\lambda$ for scenarios 1 to 7}
\medskip
