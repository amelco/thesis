...
Additionally, the results are compared with a model of constant solute uptake (based on the work of \citeonline[willigen1]) to show the differences between these two approaches.

Figure \ref[fig_C0] shows the differences in the concentration as a function of time of the three different models. 
In ZU, salt is transported to the roots by convection, causing an accumulation of solutes at root surface. As water flux towards the root starts to decrease, salt is transported slower and carried away from the roots by diffusion. 
CU presents the same behaviour with a slightly lower $C_0$ value, due to the small solute uptake value. 
The difference between ZU and CU concentrations increases as the water flux decreases.
...
Figure \ref[fig_heads2] shows the solute uptake rate (solute flux at root surface $q_{s_0}$) as a function of time of each model, and it can be seen the three uptake phases for NLU (LUP, CUP and NUP), the constant uptake rate for CU and the zero uptake rate for ZU, according to their respective equations.
...
\caption/f {Pressure ($h$), osmotic ($h_\pi$) and total ($H$) heads at root surface as a function of time for constant (CU) and zero (ZU) uptake models, for scenario 1}
\medskip

\medskip
\label[fig_heads2]
\picw=13cm \cinspic headsNLxt.pdf
\caption/f {Pressure ($h$), osmotic ($h_\pi$) and total ($H$) heads as a function of time for nonlinear (NLU) uptake model, for scenario 1}
\medskip
...
The high uptake rate at LUP, in NLU, causes a fast accumulation of solute inside the plant due to passive uptake.
According to the model assumptions, the plant does not regulate passive uptake (or the uptake caused by mass flow of water), which, for the case of NLU, is the greatest contribution to the uptake.
...
Therefore, in this situation, all solute that is extracted by the plant will contribute to a unavoidable accumulation of solute inside the plant, causing the toxicity effects of the ionic stress.The ZU model obviously does not accumulate any solute and LU model accumulates it linearly, at a constant rate.
}
...
This period is then followed by a period of diffusion of solute away from the roots which, along with the solute uptake, is responsible for the depletion of solute at the root surface.
This behaviour causes a different solute concentration profile when compared with the proposed solute uptake model (NLU).
It can be seen on figure \ref[fig_C] that the diffusion of solute away from the roots generates a higher concentration somewhere between $r_0$ and $r_m$, as opposed to NLU model, which has no solute accumulation period.}
{\tred
It is also shown that the models that consider solute uptake (CU and NLU) present a smaller concentration at $r_m$ due to the solute transport towards the root generate by the gradient caused by the solute uptake.
}
...
\caption/f {Soil solution concentration as a function of distance from axial root center for constant (CU), zero (ZU) and nonlinear (NLU) uptake models. \descc}
\medskip
...
{\tred The effects of the MM-based uptake model applied in the convection-dispersion equation are noticed in very low concentration values.
...
{\tred
The numerical proposed model is compared with the analytical model proposed by \citeonline[cushman] in Figure \ref[fig_analytic].
The analytical model has the disadvantage to deal with constant water flux only (steady-state condition).
...
We put the time to evaluate the concentration profile in the analytical model as being the same for the numerical model for scenario 1 ($t_{end}=30$ days).
We have also to put a fixed dispersion coefficient, which is crucial to the behavior showed in the figure.
In the numerical model, the dispersion coefficient changes in time and distance whilst in the analytical model, it is a constant in time and space.
To achieve the similar concentration profile showed in the figure, we had to put a value of $D=1.98 10^{-7}$ cm$^2$ s$^{-1}$, whilst in the numerical model, this coefficient ranges from XXX to XXX.
}

\medskip
\label[fig_analytic]
\picw=13cm \cinspic analytic_cushman.pdf
\caption/f {Analytical and numerical result of scenario 1 simulation }
\medskip
