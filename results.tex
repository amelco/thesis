...
According to the analysis of the differences between the linear (LU) and nonlinear (NLU) solution for the solute uptake equation showed in the last section, the simulations of scenarios 1 to 8 (Table \ref[tab_scenarios]) were made using NLU. 
...
All simulations had initial pressure head values of $-1$~m, so that both wet and dry soil conditions could be simulated.
...
which changes with
soil type, root density, initial concentration and potential transpiration
(Figure \ref[fig_C_t]).
...
thus, according to the model assumptions, an active uptake is developed and it becomes higher as the passive uptake (driven by mass flow of water) becomes lower in order to maintain the constant uptake rate (Figure \ref[fig_contrib_t]).
...
Passive uptake decreases as fast as $q_0$ and $C_0$ decreases depending on the rate of solute uptake \im{} (according to Equation \ref[eq_MM_mod] and model assumptions), therefore, the soil type, root density, initial concentration and potential transpiration also affects the time for the maximum active uptake value and, consequently, the time at which NUP starts.
...
The characteristic of the nonlinear uptake phase is a second falling rate phase for $T_r$ (as shown in Figure \ref[diff_tr]). 
...
Although changing from low to high values of potential transpiration causes changes in the length of the phases (LUP, CUP and NUP), no changes in the active and passive partitioning was detected with the variation of this parameter.
...
In a case that all model parameters of soil type, root density, ion type and solute uptake are the same, it is expected the same results in different times.
...
When $T_r\approx0$ (end of simulation), $C_0$ value is 17.88~mol~m$^{-3}$, higher than $C_{lim}$ 
%(5.97~mol~m$^{-3}$) 
...
In other words, the simulations showed that reducing $T_p$ might cause the same concentration profiles for every scenario, although at different times, if all other parameters are the same.
...
In that way, an increase in $\theta_s$ is proportionally more significant than $\theta_r$, for example, as for low water contents values the uptake rates are smaller.
...
{\tred 
The parameter $n$ showed the highest values of $\eta$ among the soil hydraulic properties, mainly for $h_{os}$ value in the end of simulation reaching the value of $-$23 for scenario 2 (Figure~\ref[sensitivity_n]).
Similar result was found by \citeonline[quirijn_sens], where the yield predictions of the evaluated model had higher sensitivity for the $n$ parameter, also with one order of magnitude higher.
Figure \ref[fig_wrcs] shows how the water retention curve, following the \citeonline[genuchten80] model, changes with the variation of 1\% in $n$ parameter.
The same value of water content is reached with higher values (more negative) of pressure head.
This brings expectations that the limiting value $H_{lim}$ would be reached at lower values of $\theta$ with higher value of solute concentration in soil water (more negative $h_\pi$).
That is what occurs in the simulations with loam soil (scenarios 1,2,3,4 and 5) but, despite the higher change in the water retention curve for the clay soil, does not occur for scenario 7 (its variation parameter is the soil type: clay).
The cause resides on the fact that the solute uptake is so low at this scenario that an apparent big change on the retention curve does not makes so much difference.
}
\medskip
\label[sensitivity_n]
...
\caption/f {Theoretical water retention curves, according to Equation \ref[eq_theta], for the selected soils from Table \ref[soils]. The lower value of $h$ matches the initial value for all simulations $h_{ini}=-1$~m. Dashed lines represent the variation of 1\% in the $n$ parameter}
\medskip
