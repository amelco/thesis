...
For the simulation scenarios, the set of input parameters $h_{ini}$ and $C_{ini}$ were chosen in a way that the simulation always starts at the linear phase, at which the water is taken up in the same rate of the potential transpiration and the solute uptake is done by mass flow of water only (passive uptake).
...
The depletion of $C_0$ starts a little after at the onset of the $T_r$ falling rate phase.
When the total head reaches its limit value $H_{lim}$, there is a reduction in $q_0$ and, consequently, an increase in $C_2$ value according to Equation \ref[eq_c2].
Therefore, the time that the constant uptake phase ($C_0<C_2$) starts depends on the rate of $q_0$ decrease.
(WE CAN SEE IT IN THE DIFFERENT SOIL TYPES)
In the constant uptake phase, the solute uptake by mass flow does not attend the plant demand. 
Thus, according to the model assumptions, an active uptake is developed and it becomes higher as the passive uptake (driven by mass flow of water) becomes lower.
This constant demand, fulfilled by active and passive uptake, is maintained until $C_0$ reaches $C_{lim}$ which, in turn, increases as $q_0$ decreases (Equation \ref[eq_clim]).
At this point, active uptake reached its maximum value and now it is limited by the concentration in the soil.
This is also when a second reduction in $T_r$ occurs (EXPLAIN WHY).



The variation in concentration over time and distance to soil hydraulic properties is shown in Figures \ref[fig_C_t](a) and \ref[fig_C_r](a).
Although the initial concentration in bulk soil is the same (10 mol m$^{-3}$), initially, the concentrations at the root surface are different for each soil type. 
...
The water retention curve for the three soil types can be seen in Figure \ref[fig_wrcs].
The constant $C_0$ values on Figure \ref[fig_C_r]

In the clay soil, $C_0$

%All simulations started with a low value of pressure head ($h=-1 m$) such that the potencial transpiration is attended ($T_r=1$) until the value of total potentail $H$ is equal to the limiting value, -150 m in our simulations.
%This is the onset of the falling rate phase for the relative transpiration ($T_r<1$). The water flux diminishes from its start value -- wich was constant because $T_r$ was also constant -- and the concentration in soil water starts to diminish (the uptake by mass flow also diminishes and it starts to become less important) because the water uptake rate is less but the solute uptake remains the same -- it is the constant solute uptake phase.
%When the concentration at root surface becomes less than $C_{lim}$, we are at the LCC phase and the uptake by mass flow is really small. 
%
%To see the differences in the model predictions we changed some input parameters...
%It can be seen in Figure \ref[fig_C_r](a) that the concentration profile is different for different soil types. Fine-textured soil retains more water and the soil reaches the residual water content faster. 
%The only situation that did not make difference was for the potential transpiration change. The final concentration profile was the same. EXPLAIN WHY
%
...
%Finer-texture soils offer more resistance to water transport and to convective solute flux.
%In the clay soil, the relative transpiration starts to reduce from its potential earlier when compared to loam or sand soil because, according to its water retention curve
%, making that the concentration at root surface for clay soil.
%The water retention characteristics are different for each solo type, reflecting in different concentration profiles.
%
...
\medskip
\label[fig_wrcs]
\picw=11cm \cinspic wrcs.pdf
\caption/f {Theoretical water retention curves, according to Equation \ref[eq_theta], for the selected simulation soils from Table \ref[soils]}
\medskip
