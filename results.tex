...
\def\diff{{\it diff}}
According to Equations \ref[eq_MM_mod] and \ref[eq_MM_linear], the difference between LU and NLU occurs only for concentration at root surface ($C_0$) values below $C_{lim}$ and, therefore, the differences between the two models (according to Equations  \ref[error_abs] and \ref[error_rel]) were calculated for this time period only. 
