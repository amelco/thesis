...
and
(2) the second due to the decrease of the solute uptake rate resulted from the change in the boundary condition when LCC starts, causing a decreasing (more negative) value for the osmotic head $h_\pi$.
...
As $H$ is not allowed to reduce below $H_{lim}$, the model predicts lower $h$ values (less negative) which leads to smaller water flux rates at the root surface $q_0$, reflected in $T_r$.
}
Therefore, this second reduction depends on the value of $q_0$, which directly affects the $C_{lim}$ value (Equation \ref[eq_clim]), and abruptly reduces the relative transpiration due to the combined water and osmotic stress.
...
%{\it QM: Nao entendi isso. Nao eh possivel um cenario assim?}
%{\tblue {\it My answer: The reason is that \clim{} value can not be chosen. It is numerically calculated by Equation \ref[eq_clim] and its value only changes with $q_0$ variation.}}
the value of $C_{lim}$ is computed by Equation \ref[eq_clim] and it varies only with $q_0$ variation (it is not an input parameter).
...
Figure \ref[fig_wrcs](a) shows how the water retention curve, following the \citeonline[genuchten80] model, changes with the {\tred variation of 1\% in $n$ parameter. {\it QM: N afeta ambas a retencaoo e a condutividade.Eh importante dizer isso tambem.}}
...
CU results in a similar prediction with a slightly lower $C_0$ value due to {\tred a small solute uptake value {\it QM: Qual eh esse valor? Isso estah claramente no M\&M?}}. 
...
The higher the value of the constant uptake rate and the lower $T_p$ in CU, less solute will predicted to accumulate at the root surface by the CU model and more similar CU and NLU predictions will be, as can be seen in the gray line of Figure \ref[fig_C0].

\def\descc{Gray line is CU with the constant of solute uptake rate {\tred 10 times higher {\it QM: Mas qual eh o valor original?}}}
