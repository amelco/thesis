...
Therefore, to get similar concentration profiles, the analytical model had a value of $D=1.98 \cdot 10^{-7}$ cm$^2$ s$^{-1}$ (or $1.98\cdot10^{-11}$ m$^2$ s$^{-1}$) whereas the numerical model had dispersion coefficients that ranged from $4.13 \cdot 10^{-10}$ to $1.25\cdot10^{-12}$ m$^2$ s$^{-1}$ at the root surface.
